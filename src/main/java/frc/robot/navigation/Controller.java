...
public class Controller {
    ...
    /*
     * private double LeftOriginal; //target Left wheels speed private double
     * RightOriginal; //target right wheels speed private double LeftFinal; //target
     * Left wheels speed private double RightFinal; //target right wheels speed
     */
    ...
    public boolean controlLoop(double leftChange, double rightChange, double heading) {
        ...
        while (robotPosition.distFrom(genPath.get(genPath.size() - 1)) > 6) {
            ...
            int nextLookAheadIndex = genPath.findLookAheadIndex(this.lDistance, this.robotPosition,
                    this.lookAheadPointIndex);

            if (this.lookAheadPointIndex != nextLookAheadIndex) {
                System.err
                        .println("Look Ahead Point changed: " + this.lookAheadPointIndex + " to " + nextLookAheadIndex);
            }

            this.lookAheadPointIndex = nextLookAheadIndex;
            this.lookAheadPoint = genPath.get(nextLookAheadIndex);

            // this.lookAheadPoint = genPath.get(genPath.findLookAheadIndex(this.lDistance,
            // this.robotPosition,
            // this.lookAheadPointIndex));
             ...
             * this.targetAccelerationLeft = ( ( (this.LeftFinal * this.LeftFinal) -
             * (this.LeftOriginal * this.LeftOriginal) ) / (2 * dist) );
             * this.targetAccelerationRight = ( ( (this.RightFinal * this.RightFinal) -
             * (this.RightOriginal * this.RightOriginal) ) / (2 * dist) );
             * 
             * this.ffL = this.kV * this.LeftFinal + this.kA * this.targetAccelerationLeft;
             * this.ffR = this.kV * this.RightFinal + this.kA *
             * this.targetAccelerationRight; this.fbL = this.kP * (this.LeftFinal - lSpeed);
             * this.fbR = this.kP * (this.RightFinal - rSpeed);
             * 
             * this.Left = (this.ffL + this.fbL); this.Right = (this.ffR + this.fbR);
             * this.LeftOriginal = this.RightFinal; this.RightOriginal = this.RightFinal;
             * 
             * this.wV.put("Left", Left); this.wV.put("Right", Right);
             * 
             * return this.wV;
             */
        }
